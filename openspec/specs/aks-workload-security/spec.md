# aks-workload-security Specification

## Purpose
Zero-trust identity and network posture for the AKS platform: each service authenticates via its own federated Managed Identity scoped to only the Key Vault secrets it needs, the cluster's OIDC issuer and Workload Identity are Terraform-managed cluster attributes, and Key Vault plus the container registry are reachable only via private endpoint inside the platform VNet — so no plaintext credential ever lives in git or a Kubernetes manifest.
