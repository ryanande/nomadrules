# migration-observability Specification

## Purpose
Best-effort observability for the ephemeral migration runner: push-based Prometheus metrics (via Pushgateway) and OTLP traces (to Jaeger), wired only when their endpoints are configured so a missing telemetry backend never fails a migration.
