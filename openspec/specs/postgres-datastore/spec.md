# postgres-datastore Specification

## Purpose
Azure Database for PostgreSQL Flexible Server as the shared datastore for all NomadRules services — network-isolated to the AKS cluster's VNet, with connection credentials sourced from Key Vault rather than a plaintext literal, and local development parity via a containerized Postgres.
