...
## Purpose
Event-driven delivery worker: computes due renewal alerts (60/30/7 days) and weekly law-change digests from the shared schema, renders and sends them via Resend, and records every send idempotently (DB reserve rows + a provider-side Idempotency-Key) so no subscriber is ever double-emailed.
